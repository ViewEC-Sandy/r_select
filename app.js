...
const FIELDS = [
  ...
  ['taiwanUrl','台灣樂天URL','url','import'],['rianyouUrl','日安URL','url','import'],['japanUrl','日本URL','url','import'],['rtwBaseSku','RTWBase SKU','text','import'],['storeCode','店鋪編號','text','editable'],['storeName','店鋪名','text','calculated'],
  ...
  ['suggestedPrice30TWD','樂天目標利潤建議售價(TWD)','number','calculated'],
  ...
  ['rianyouSuggestedPrice30TWD','日安目標利潤建議售價(TWD)','number','calculated'],
...
function getStore(base){const explicit=cleanText(base.storeCode).toUpperCase();const code=explicit||extractStoreCode(base.productManagementId,base.specManagementId);return code?storeMap.get(code):null}
function effectiveTargetRate(store,platform='rakuten'){
  const key=platform==='rianyou'?'rianyouTargetProfitRate':'rakutenTargetProfitRate';
  const fallback=platform==='rianyou'?params.rianyouTargetProfitRate:params.targetProfitRate;
  const raw=store?.[key];
  return raw!==null&&raw!==undefined&&raw!==''&&Number.isFinite(Number(raw))?Number(raw):fallback;
}
...
function compute(base){
  ...
  const storeCode=cleanText(p.storeCode).toUpperCase()||store?.code||extractStoreCode(p.productManagementId,p.specManagementId);
  ...
  const rakutenTarget=effectiveTargetRate(store,'rakuten');
  const rianyouTarget=effectiveTargetRate(store,'rianyou');
  const rakuten=platformFinancial(
    ov.manualPriceTWD?n(p.manualPriceTWD):null,weight,productCost,domestic,fixed,
    params.platformFeeRate,rakutenTarget,params.freeShippingTWD
  );
  const rianyou=platformFinancial(
    ov.rianyouPriceTWD?n(p.rianyouPriceTWD):null,weight,productCost,domestic,fixed,
    params.rianyouPlatformFeeRate,rianyouTarget,params.rianyouFreeShippingTWD
  );
  ...
  return {...p,
    ...
    platformFeeTWD:rakuten.fee,profitTWD:rakuten.profit,profitRate:rakuten.margin,suggestedPrice30TWD:rakuten.suggested,rakutenEffectiveTargetRate:rakutenTarget,
    rianyouPriceTWD:rianyou.sale,rianyouCustomerShippingTWD:rianyou.customer,rianyouGrossReceivedTWD:rianyou.gross,
    rianyouPlatformFeeTWD:rianyou.fee,rianyouProfitTWD:rianyou.profit,rianyouProfitRate:rianyou.margin,rianyouSuggestedPrice30TWD:rianyou.suggested,rianyouEffectiveTargetRate:rianyouTarget,
...
function promotionPlatformConfig(platform='rakuten'){
  return platform==='rianyou'
    ?{priceKey:'rianyouPriceTWD',feeRate:params.rianyouPlatformFeeRate,freeShippingTWD:params.rianyouFreeShippingTWD,label:'日安優物'}
    :{priceKey:'manualPriceTWD',feeRate:params.platformFeeRate,freeShippingTWD:params.freeShippingTWD,label:'樂天'};
}
function promotionFinancial(p,originalPrice,discountPercent,platform='rakuten'){
  const cfg=promotionPlatformConfig(platform),rate=Math.max(0,Math.min(99.9,n(discountPercent)))/100;
  const promoPrice=Math.round(n(originalPrice)*(1-rate));
  const customer=promoPrice>=cfg.freeShippingTWD?0:ceilKg(p.weightG)*params.customerShippingPerKgTWD;
  const gross=promoPrice+customer,fee=promoPrice*cfg.feeRate;
  ...
  return {promoPrice,customer,gross,fee,profit,margin:gross&&profit!==null?profit/gross:null};
}
function minimumSafePromoPrice(p,minMargin,platform='rakuten'){
  const cfg=promotionPlatformConfig(platform),q=Math.max(0,Math.min(.8,n(minMargin))),cost=n(p.productCostTWD)+n(p.domesticShippingTWD)+n(p.fixedLogisticsCostTWD),denom=1-cfg.feeRate-q;
  if(denom<=0)return null;
  const ship=ceilKg(p.weightG)*params.customerShippingPerKgTWD;
  const nonFree=(cost-ship*(1-q))/denom;
  const free=Math.max(cfg.freeShippingTWD,cost/denom);
  const candidates=[];
  if(nonFree<cfg.freeShippingTWD)candidates.push(Math.max(0,nonFree));
  candidates.push(free);
  let y=Math.ceil(Math.min(...candidates));
  // 因促銷價採四捨五入，最後以完整損益驗證並向上微調。
  for(let i=0;i<10000;i++,y++){
    const customer=y>=cfg.freeShippingTWD?0:ship,gross=y+customer,fee=y*cfg.feeRate,profit=gross-cost-fee,margin=gross?profit/gross:null;
    if(margin!==null&&margin+1e-12>=q)return y;
  }
  return null;
}
function promotionAnalysisProduct(p,discountPercent,minMargin,platform='rakuten'){
  const cfg=promotionPlatformConfig(platform),original=n(p[cfg.priceKey]),current=promotionFinancial(p,original,0,platform),f=promotionFinancial(p,original,discountPercent,platform),rate=Math.max(0,Math.min(99.9,n(discountPercent)))/100;
  const safePromo=minimumSafePromoPrice(p,minMargin,platform);
  let safeOriginal=safePromo===null||rate>=1?null:Math.ceil(safePromo/(1-rate));
  if(safeOriginal!==null){for(let i=0;i<1000;i++,safeOriginal++){const check=promotionFinancial(p,safeOriginal,discountPercent,platform);if(check.margin!==null&&check.margin+1e-12>=minMargin)break}}
  const adjust=safeOriginal===null?null:Math.max(0,safeOriginal-original),adjustPct=original&&adjust!==null?adjust/original:null;
  const maxDiscount=safePromo!==null&&original>0&&safePromo<=original?Math.max(0,Math.min(.999,1-safePromo/original)):0;
  const status=f.margin===null?'無法計算':f.profit<0?'無法做促銷｜負利潤':f.margin<minMargin?`無法做促銷｜低於 ${(minMargin*100).toFixed(1)}%`:'可促銷';
  return {...p,promotionPlatform:platform,discountPercent:rate,minPromotionMargin:minMargin,originalPriceTWD:original,currentProfitTWD:current.profit,currentProfitRate:current.margin,discountedPriceTWD:f.promoPrice,discountedCustomerShippingTWD:f.customer,discountedGrossTWD:f.gross,discountedPlatformFeeTWD:f.fee,discountedProfitTWD:f.profit,discountedProfitRate:f.margin,promotionStatus:status,safeOriginalPriceTWD:safeOriginal,priceAdjustmentTWD:adjust,priceAdjustmentRate:adjustPct,maxSafeDiscountRate:maxDiscount};
}
function promotionFilteredResults(){
  const mode=$('discountStatusFilter')?.value||'all';
  if(mode==='all')return [...discountResults];
  if(mode==='eligible')return discountResults.filter(r=>r.discountedProfitRate!==null&&r.discountedProfitRate>=r.minPromotionMargin);
  if(mode==='ineligible')return discountResults.filter(r=>r.discountedProfitRate===null||r.discountedProfitRate<r.minPromotionMargin);
  if(mode==='negative')return discountResults.filter(r=>n(r.discountedProfitTWD)<0);
  if(mode==='low')return discountResults.filter(r=>n(r.discountedProfitTWD)>=0&&r.discountedProfitRate!==null&&r.discountedProfitRate<r.minPromotionMargin);
  return [...discountResults];
}

function renderDiscountResults(){
  const percent=n($('discountPercent').value),minMargin=Math.max(0,Math.min(80,n($('discountMinMargin')?.value||10)))/100,platform=$('discountPlatform')?.value||'rakuten';
  const selected=products.filter(p=>selectedProductIds.has(p.id));discountResults=selected.map(p=>promotionAnalysisProduct(p,percent,minMargin,platform));
  const shown=promotionFilteredResults();
  $('discountTableBody').innerHTML=shown.map(r=>`<tr class="${r.discountedProfitRate===null||r.discountedProfitRate<minMargin?'promotion-danger-row':''}"><td>${esc(r.specManagementId||'')}</td><td>${esc(r.storeCode||'')}</td><td title="${esc(r.title||'')}">${esc(shortTitle(r.title||''))}</td><td>${formatInteger(r.originalPriceTWD)}</td><td class="${r.currentProfitRate!==null&&r.currentProfitRate<minMargin?'danger-text':''}">${r.currentProfitRate===null?'':(r.currentProfitRate*100).toFixed(1)+'%'}</td><td>${formatInteger(r.discountedPriceTWD)}</td><td>${r.discountedProfitTWD===null?'':formatInteger(r.discountedProfitTWD)}</td><td class="${r.discountedProfitRate!==null&&r.discountedProfitRate<minMargin?'danger-text':''}">${r.discountedProfitRate===null?'':(r.discountedProfitRate*100).toFixed(1)+'%'}</td><td class="${r.promotionStatus.startsWith('無法')?'danger-text':''}">${esc(r.promotionStatus)}</td><td>${r.safeOriginalPriceTWD===null?'':formatInteger(r.safeOriginalPriceTWD)}</td><td>${r.priceAdjustmentTWD===null?'':(r.priceAdjustmentTWD>0?'+':'')+formatInteger(r.priceAdjustmentTWD)}</td><td>${r.priceAdjustmentRate===null?'':(r.priceAdjustmentRate*100).toFixed(1)+'%'}</td><td>${(r.maxSafeDiscountRate*100).toFixed(1)}% OFF</td></tr>`).join('');
  const valid=discountResults.filter(r=>Number.isFinite(r.discountedProfitRate)),avg=valid.length?valid.reduce((s,r)=>s+r.discountedProfitRate,0)/valid.length:0,negative=discountResults.filter(r=>n(r.discountedProfitTWD)<0).length,ineligible=discountResults.filter(r=>r.discountedProfitRate===null||r.discountedProfitRate<minMargin).length,eligible=discountResults.length-ineligible;
  $('discountSummary').innerHTML=`<span>試算商品<strong>${formatInteger(discountResults.length)}</strong></span><span>可促銷<strong>${formatInteger(eligible)}</strong></span><span class="danger-text">無法促銷<strong>${formatInteger(ineligible)}</strong></span><span class="danger-text">負利潤<strong>${formatInteger(negative)}</strong></span><span>平均折後利潤率<strong>${(avg*100).toFixed(1)}%</strong></span>`;
  if($('discountShownCount'))$('discountShownCount').textContent=`目前顯示 ${shown.length} 筆`;
}
...
function exportDiscountResults(filteredOnly=false){const rows=(filteredOnly?promotionFilteredResults():discountResults);if(!rows.length)return toast('目前沒有可下載的試算結果');const platform=promotionPlatformConfig($('discountPlatform')?.value||'rakuten').label;const out=rows.map(r=>({'平台':platform,'店鋪編號':r.storeCode||'','商品規格管理編號':r.specManagementId||'','商品管理編號':r.productManagementId||'','商品標題':r.title||'','促銷折扣率':r.discountPercent,'最低促銷利潤率':r.minPromotionMargin,'目前售價(TWD)':r.originalPriceTWD,'目前利潤率':r.currentProfitRate??'','促銷售價(TWD)':r.discountedPriceTWD,'促銷客收運費(TWD)':r.discountedCustomerShippingTWD,'促銷平台費(TWD)':Math.round(n(r.discountedPlatformFeeTWD)),'促銷後利潤(TWD)':r.discountedProfitTWD===null?'':Math.round(r.discountedProfitTWD),'促銷後利潤率':r.discountedProfitRate??'','活動判定':r.promotionStatus,'促銷安全原價(TWD)':r.safeOriginalPriceTWD??'','需調整金額(TWD)':r.priceAdjustmentTWD??'','需調整幅度':r.priceAdjustmentRate??'','最深可行折扣':r.maxSafeDiscountRate??''}));const wb=XLSX.utils.book_new();XLSX.utils.book_append_sheet(wb,XLSX.utils.json_to_sheet(out),'促銷可行性分析');XLSX.writeFile(wb,`促銷可行性分析_${filteredOnly?'目前篩選_':'全部_'}${new Date().toISOString().slice(0,10)}.xlsx`)}
...
function renderProductForm(p={}){
  ...
  $('productFields').innerHTML=platformHtml+FIELDS.filter(([k])=>!['conversionRate','storeName','domesticShippingJPY'].includes(k)).map(([k,l,t,mode])=>{
    const v=computed[k]??'';
    if(k==='storeCode'){const current=cleanText(v).toUpperCase();const opts=['<option value="">未指定（依商品編號推測）</option>',...stores.map(s=>`<option value="${esc(s.code)}" ${current===cleanText(s.code).toUpperCase()?'selected':''}>${esc(s.code)}｜${esc(s.name||'未命名店鋪')}</option>`)];if(current&&!stores.some(s=>cleanText(s.code).toUpperCase()===current))opts.push(`<option value="${esc(current)}" selected>${esc(current)}｜目前資料（店舖管理未建立）</option>`);return `<label>${l}<select name="${k}" data-field-key="${k}" data-field-mode="editable">${opts.join('')}</select></label>`}
...
function readProductFormDraft(){
  ...
  FIELDS.forEach(([k,,t])=>{
    if(['conversionRate','storeName','domesticShippingJPY'].includes(k))return;
...
async function saveProduct(form){
  ...
  FIELDS.forEach(([k,,t])=>{if(['conversionRate','storeName','domesticShippingJPY'].includes(k))return;const raw=fd.get(k);if(raw===null)return;data[k]=t==='number'||t==='percent'?(raw===''?null:Number(raw)):t==='boolean'?raw==='true':String(raw)});
  data.storeCode=cleanText(data.storeCode).toUpperCase();data.taiwanUrl=validUrl(data.taiwanUrl);data.rianyouUrl=validUrl(data.rianyouUrl);data.japanUrl=validUrl(data.japanUrl);data.platformData={};PLATFORMS.forEach(x=>{data.platformData[x.id]={enabled:fd.get(`platform_${x.id}_enabled`)==='on',price:fd.get(`platform_${x.id}_price`)===''?null:Number(fd.get(`platform_${x.id}_price`)),active:fd.get(`platform_${x.id}_active`)==='true',note:String(fd.get(`platform_${x.id}_note`)||'')}});data.title=String(data.title||'').slice(0,100);data.updatedAt=serverTimestamp();if(!id)data.createdAt=serverTimestamp();const ref=id?doc(db,'products',id):doc(collection(db,'products'));await setDoc(ref,data,{merge:true});toast('商品已儲存');$('productDialog').close();await loadAll()
}
...
const PARTIAL_PRODUCT_UPDATE_FIELDS=[
  {key:'title',aliases:['商品標題','商品名稱','商品名'],type:'text',label:'商品標題'},
  {key:'spec1',aliases:['規格1'],type:'text',label:'規格1'},
  ...
  {key:'unitsSold',aliases:['售出單位','銷售商品數','銷售數'],type:'number',label:'銷售商品數'},
  {key:'orderCount',aliases:['訂單計數','銷售訂單數'],type:'number',label:'銷售訂單數'}
];

function partialRawValue(row,aliases){
  for(const alias of aliases){
    const actual=findHeader(row,[alias]);
...
function partialBoolean(v){
  const s=cleanText(v).toLowerCase();
  if(['true','1','yes','y','是','上架','啟用','開啟'].includes(s))return true;
  if(['false','0','no','n','否','下架','停用','關閉'].includes(s))return false;
  return null;
}

async function importPartialProductUpdate(raw,file,selectedPlatform){
  setImportProgress(`指定欄位更新：解析 ${raw.length.toLocaleString()} 列…`,25);
  await yieldToUI();

  const specAliases=['商品規格管理編號','SKU','sku'];
  const productAliases=['商品管理編號','商品管理編號 (Base SKU)','商品管理編號(Base SKU)','Base SKU','商品編號'];
  ...
  const detectedFields=PARTIAL_PRODUCT_UPDATE_FIELDS.filter(def=>
    raw.some(row=>findHeader(row,def.aliases)!==null)
  );
  if(!detectedFields.length){
    throw new Error('沒有偵測到可更新欄位。請在兩個識別編號之外加入至少一個要更新的欄位');
  }

  const productBySpec=new Map(
    products.filter(p=>cleanText(p.specManagementId))
      .map(p=>[cleanText(p.specManagementId).toLowerCase(),p])
  );
  ...
  for(let i=0;i<raw.length;i++){
    ...
    const old=productBySpec.get(spec.toLowerCase());
    if(!old){
    ...
    for(const def of detectedFields){
      const rawValue=partialRawValue(row,def.aliases);
      ...
      if(def.type==='number'){
        ...
        if(value===null){
          errors.push(`第 ${i+2} 列：${def.label}「${rawValue}」不是有效數字`);
          continue;
        }
      }else if(def.type==='boolean'){
        value=partialBoolean(rawValue);
        if(value===null){
          errors.push(`第 ${i+2} 列：${def.label}「${rawValue}」無法辨識 true/false`);
          continue;
        }
      }else if(def.type==='url'){
        value=validUrl(rawValue);
        if(!value&&cleanText(rawValue)){
          errors.push(`第 ${i+2} 列：${def.label}「${rawValue}」不是有效網址`);
          continue;
        }
      }else{
      ...
      patch[def.key]=value;
      changed++;
      fieldCounts[def.label]=(fieldCounts[def.label]||0)+1;
    }
  ...
  if(!updates.length){
    ...
    throw new Error(`沒有可更新資料${preview?`：${preview}`:''}`);
  }

  if(errors.length){
    const preview=errors.slice(0,8).join('\n');
    if(!confirm(`發現 ${errors.length} 筆不會更新的資料：\n\n${preview}${errors.length>8?`\n…另有 ${errors.length-8} 筆`:''}\n\n其餘 ${updates.length} 筆仍要繼續嗎？`)){
      setImportProgress('已取消指定欄位更新',0);
  ...
  for(let start=0,batchNo=1;start<updates.length;start+=BATCH_SIZE,batchNo++){
    const chunk=updates.slice(start,start+BATCH_SIZE);
    setImportProgress(
      `指定欄位更新：正在寫入 ${Math.min(start+chunk.length,updates.length).toLocaleString()} / ${updates.length.toLocaleString()} 筆`,
      35+(batchNo/totalBatches)*55
    );
    await yieldToUI();
    const batch=writeBatch(db);
  ...
  await addDoc(collection(db,'imports'),{
    type:'products_partial_update',
    fileName:file.name,
    rowCount:raw.length,
    successCount:updates.length,
    skippedCount:blankSkipped+errors.length,
  ...
  const fieldsSummary=Object.entries(fieldCounts).map(([k,v])=>`${k} ${v}`).join('、');
  setImportProgress(
    `指定欄位更新完成：成功 ${updates.length.toLocaleString()} 筆，跳過 ${blankSkipped.toLocaleString()} 筆，錯誤 ${errors.length.toLocaleString()} 筆；${fieldsSummary}`,
    96
  );
  toast('指定欄位更新完成');
  ...
  await loadAll();
  setImportProgress(
    `完成：成功 ${updates.length.toLocaleString()} 筆，跳過 ${blankSkipped.toLocaleString()} 筆，錯誤 ${errors.length.toLocaleString()} 筆`,
    100
  );
  return {success:updates.length,skipped:blankSkipped,errors:errors.length};
}
...
async function importExcel(){
  ...
  try{
    ...
    if(importStrategy==='partial'){
      await importPartialProductUpdate(raw,file,selectedPlatform);
      return;
    }
    const isShopline=selectedPlatform==='rianyou_shopify';
...
function renderStores(){const q=cleanText($('storeSearch').value).toLowerCase();const list=stores.filter(s=>!q||[s.code,s.name,s.url,s.shippingJPY,s.rakutenTargetProfitRate,s.rianyouTargetProfitRate].some(v=>String(v||'').toLowerCase().includes(q)));$('storeTableBody').innerHTML=list.map(s=>`<tr><td>${esc(s.code)}</td><td>${esc(s.name||'')}</td><td>${format('japanUrl',s.url)}</td><td>${esc(s.shippingJPY??params.domesticShippingJPY)}</td><td>${s.rakutenTargetProfitRate===null||s.rakutenTargetProfitRate===undefined?'系統預設':(n(s.rakutenTargetProfitRate)*100).toFixed(1)+'%'}</td><td>${s.rianyouTargetProfitRate===null||s.rianyouTargetProfitRate===undefined?'系統預設':(n(s.rianyouTargetProfitRate)*100).toFixed(1)+'%'}</td><td class="action-cell"><button type="button" data-store-edit="${esc(s.id)}">編輯</button><button type="button" class="secondary" data-store-delete="${esc(s.id)}">刪除</button></td></tr>`).join('');$('storeCount').textContent=`共 ${list.length} 間店鋪`}
function openStoreForm(store={}){$('storeId').value=store.id||'';$('storeCode').value=store.code||'';$('storeName').value=store.name||'';$('storeUrl').value=store.url||'';$('storeShipping').value=store.shippingJPY??params.domesticShippingJPY;$('storeRakutenTarget').value=store.rakutenTargetProfitRate===null||store.rakutenTargetProfitRate===undefined?'':(n(store.rakutenTargetProfitRate)*100);$('storeRianyouTarget').value=store.rianyouTargetProfitRate===null||store.rianyouTargetProfitRate===undefined?'':(n(store.rianyouTargetProfitRate)*100);$('storeFormTitle').textContent=store.id?'編輯店鋪':'新增店鋪';$('storeEditDialog').showModal()}
async function saveStore(form){const fd=new FormData(form),id=$('storeId').value,code=cleanText(fd.get('code')).toUpperCase();if(!/^R\d{1,4}$/i.test(code))return toast('店鋪編號格式需為 R 加數字，例如 R60');const rakutenTargetRaw=cleanText(fd.get('rakutenTargetProfitRate')),rianyouTargetRaw=cleanText(fd.get('rianyouTargetProfitRate'));const data={code,name:cleanText(fd.get('name')),url:validUrl(fd.get('url')),shippingJPY:n(fd.get('shippingJPY'))||params.domesticShippingJPY,rakutenTargetProfitRate:rakutenTargetRaw===''?null:Math.max(0,Math.min(.8,n(rakutenTargetRaw)/100)),rianyouTargetProfitRate:rianyouTargetRaw===''?null:Math.max(0,Math.min(.8,n(rianyouTargetRaw)/100)),updatedAt:serverTimestamp()};if(!id)data.createdAt=serverTimestamp();await setDoc(doc(db,'stores',id||code),data,{merge:true});$('storeEditDialog').close();await loadAll();renderStores();toast('店鋪已儲存')}
...
function pricingSuggested(p,targetOverride=null,platform='rakuten'){
  ...
  const store=getStore(p),defaultTarget=effectiveTargetRate(store,platform);
...
function resetParamsProgress(){
  const panel=$('paramsProgressPanel');
  const track=panel?.querySelector('[role="progressbar"]');
  panel?.classList.remove('is-done','is-error');
  if($('paramsProgressBar')) $('paramsProgressBar').style.width='0%';
  if(track) track.setAttribute('aria-valuenow','0');
  if($('paramsProgressPercent')) $('paramsProgressPercent').textContent='0%';
  if($('paramsProgressCount')) $('paramsProgressCount').textContent=`0 / ${products.length}`;
