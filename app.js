...
let pricingCart=[];
let pricingHeadState={key:'',value:''};
...
function pricingMarginAtPrice(p,salePrice){
  ...
  const profit=gross-productCost-domestic-ship-fee;return gross?profit/gross:null;
}
function pricingRows(){
  const q=cleanText($('pricingSearch')?.value).toLowerCase(),target=pricingTargetRate();
  let rows=products.filter(p=>!q||[p.specManagementId,p.productManagementId,p.title].some(v=>cleanText(v).toLowerCase().includes(q))).map(p=>{
  ...
  const {key,value}=pricingHeadState;if(!key||!value)return rows;
  if(key==='method')return rows.filter(r=>r.ship.method===value);
  if(key==='margin'&&['belowTarget','atTarget','negative'].includes(value))return rows.filter(r=>r.currentMargin!==null&&(value==='belowTarget'?r.currentMargin<target:value==='atTarget'?r.currentMargin>=target:r.currentMargin<0));
  if(key==='diff'&&['raise','lower'].includes(value))return rows.filter(r=>r.diff!==null&&(value==='raise'?r.diff>0:r.diff<=0));
  const dir=value==='asc'?1:-1;
  const get=r=>({spec:cleanText(r.p.specManagementId),title:cleanText(r.p.title),priceJPY:n(r.p.priceJPY),weight:n(r.p.weightG),shipCost:r.ship.cost??Infinity,customer:r.currentCustomer,currentPrice:r.current,margin:r.currentMargin??-Infinity,suggested:r.suggested??Infinity,diff:r.diff??0}[key]);
  if(['asc','desc'].includes(value))rows.sort((a,b)=>{const av=get(a),bv=get(b);return typeof av==='string'?av.localeCompare(bv,'zh-Hant')*dir:(av-bv)*dir});
  return rows;
}
function renderPricingPage(){
  ...
  if($('pricingAvgCurrentNote'))$('pricingAvgCurrentNote').textContent=`依目前篩選商品計算｜此表單免運門檻 ${formatInteger(pricingFreeShipping())} 元`;
