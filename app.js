...
function renderPricingPage(){
  if(!$('pricingTableBody'))return;const q=cleanText($('pricingSearch')?.value).toLowerCase(),target=pricingTargetRate();
  ...
  $('pricingTableBody').innerHTML=list.map(p=>{const ship=pricingShipping(p.weightG),suggested=pricingSuggested(p,target),current=n(p.manualPriceTWD),currentCustomer=pricingCustomerShipping(p.weightG,current),currentMargin=pricingMarginAtPrice(p,current),diff=suggested===null?'':suggested-current;return `<tr><td>${esc(p.specManagementId||'')}</td><td>${esc(shortTitle(p.title||''))}</td><td>${formatInteger(p.priceJPY)}</td><td>${formatInteger(p.weightG)}</td><td>${esc(ship.method)}</td><td>${ship.cost===null?'超過級距':formatInteger(ship.cost)}</td><td>${formatInteger(currentCustomer)}</td><td>${formatInteger(current)}</td><td>${currentMargin===null?'—':(currentMargin*100).toFixed(1)+'%'}</td><td><strong>${suggested===null?'—':formatInteger(suggested)}</strong><small class="muted"> ${(target*100).toFixed(1)}%</small></td><td>${suggested===null?'—':(diff>0?'↑ +':diff<0?'↓ ':'')+formatInteger(diff)}</td></tr>`}).join('')||'<tr><td colspan="11" class="muted">沒有符合商品</td></tr>';
  renderPricingCart();
}
