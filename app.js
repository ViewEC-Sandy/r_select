...
function pricingShipping(weightG){
  ...
  if(w<600)return {method:'A物流',cost:w<=1000?params.uniFirstKgTWD:params.uniFirstKgTWD+Math.ceil((w-1000)/500)*params.uniEachHalfKgTWD};
  const kg=w/1000;if(kg>13)return {method:'B物流',cost:null};
  const tier=params.tiers.find(([max])=>kg<=max);return {method:'B物流',cost:tier?round(tier[1]*params.nisshinRate*params.nisshinDiscount+params.nisshinFixedFeeTWD):null};
}
function pricingCustomerShipping(weightG,priceTWD){return n(priceTWD)>=params.freeShippingTWD?0:(n(weightG)/1000)*params.customerShippingPerKgTWD}
function pricingSuggested(p){
  ...
  const target=params.targetProfitRate,denom=1-params.platformFeeRate-target;if(denom<=0)return null;
  const customer=(weight/1000)*params.customerShippingPerKgTWD;
  let candidate=(productCost+domestic+ship-customer*(1-target))/denom;
  if(candidate>=params.freeShippingTWD)candidate=(productCost+domestic+ship)/denom;
  return round(candidate);
}
function renderPricingPage(){
  if(!$('pricingTableBody'))return;const q=cleanText($('pricingSearch')?.value).toLowerCase();
  const list=products.filter(p=>!q||[p.specManagementId,p.productManagementId,p.title].some(v=>cleanText(v).toLowerCase().includes(q)));
  $('pricingTableBody').innerHTML=list.map(p=>{const ship=pricingShipping(p.weightG),suggested=pricingSuggested(p),current=n(p.manualPriceTWD),customer=pricingCustomerShipping(p.weightG,suggested||current),diff=suggested===null?'':suggested-current;return `<tr><td>${esc(p.specManagementId||'')}</td><td>${esc(shortTitle(p.title||''))}</td><td>${formatInteger(p.priceJPY)}</td><td>${formatInteger(p.weightG)}</td><td>${esc(ship.method)}</td><td>${ship.cost===null?'超過級距':formatInteger(ship.cost)}</td><td>${formatInteger(customer)}</td><td>${formatInteger(current)}</td><td><strong>${suggested===null?'—':formatInteger(suggested)}</strong></td><td>${suggested===null?'—':(diff>0?'↑ +':diff<0?'↓ ':'')+formatInteger(diff)}</td></tr>`}).join('')||'<tr><td colspan="10" class="muted">沒有符合商品</td></tr>';
  if($('pricingProductSelect'))$('pricingProductSelect').innerHTML='<option value="">選擇商品</option>'+products.map(p=>`<option value="${esc(p.id)}">${esc(p.specManagementId||p.productManagementId||'')}｜${esc(shortTitle(p.title||''))}</option>`).join('');
  renderPricingCart();
}
function renderPricingCart(){if(!$('pricingCartBody'))return;$('pricingCartBody').innerHTML=pricingCart.map((x,i)=>{const p=products.find(y=>y.id===x.id);if(!p)return'';const price=n(p.manualPriceTWD)||n(pricingSuggested(p));return `<tr><td>${esc(p.specManagementId||p.title||'')}</td><td>${formatInteger(price)}</td><td>${formatInteger(p.weightG)}</td><td>${x.qty}</td><td>${formatInteger(price*x.qty)}</td><td><button type="button" class="secondary" data-pricing-remove="${i}">移除</button></td></tr>`}).join('')||'<tr><td colspan="6" class="muted">尚未加入商品</td></tr>';renderPricingSummary()}
function renderPricingSummary(){if(!$('pricingSummary'))return;if(!pricingCart.length){$('pricingSummary').textContent='請先加入商品。';return}let sales=0,cost=0,domestic=0,weight=0;for(const x of pricingCart){const p=products.find(y=>y.id===x.id);if(!p)continue;const price=n(p.manualPriceTWD)||n(pricingSuggested(p)),store=getStore(p);sales+=price*x.qty;cost+=n(p.priceJPY)*params.productCostRate*x.qty;domestic+=(n(p.priceJPY)>=params.freeDomesticJPY?0:(n(store?.shippingJPY)||params.domesticShippingJPY)*params.productCostRate)*x.qty;weight+=n(p.weightG)*x.qty}const ship=pricingShipping(weight),customer=pricingCustomerShipping(weight,sales),fee=sales*params.platformFeeRate,profit=ship.cost===null?null:sales+customer-cost-domestic-ship.cost-fee,margin=profit===null?null:profit/(sales+customer);$('pricingSummary').innerHTML=`<div class="overview-kpis"><article class="card stat"><span>商品總額</span><strong>${formatInteger(sales)}</strong></article><article class="card stat"><span>總重量</span><strong>${formatInteger(weight)}g</strong></article><article class="card stat"><span>實際物流</span><strong>${ship.cost===null?'超過級距':formatInteger(ship.cost)}</strong><small>${esc(ship.method)}</small></article><article class="card stat"><span>客收運費</span><strong>${formatInteger(customer)}</strong></article><article class="card stat"><span>預估淨利</span><strong>${profit===null?'—':formatInteger(profit)}</strong></article><article class="card stat"><span>訂單利潤率</span><strong>${margin===null?'—':(margin*100).toFixed(1)+'%'}</strong><small>${margin===null?'':margin>=params.targetProfitRate?'✓ 達標':'⚠ 低於目標'}</small></article></div>`}
